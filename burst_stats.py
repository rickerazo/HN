...
if plot0 == 'True':
	...
	plt.xlabel(r'$\bar{g}P$ (nS)',fontsize=65,weight='bold')
...
file_name = 'BD_sorteddata.xlsx'
sheet=2
sorted_gp = pd.read_excel(io=file_name, sheet_name=sheet, usecols='A')
sorted_gp = sorted_gp.values
sorted_gp = sorted_gp.flatten()
sorted_BD = pd.read_excel(io=file_name, sheet_name=sheet, usecols='B')
sorted_BD = sorted_BD.values
sorted_BD = sorted_BD.flatten()
################## LINEAR REGRESSION 
...
plt.text(1,5,'r='+str(rvalue),fontsize=25)
plt.text(1,4.5,'p='+str(pvalue),fontsize=25)
plt.text(1,4,'std error='+str(stderr),fontsize=25)

# plt.text()

plt.title(r'Linear regression, $I_{pump}^{max}$= 0.2')
plt.ylabel(label)
plt.xlabel(r'$\bar{g}P$ (nS)')
...
def cubic_poly(gp,a,b,c,d):
	A = a*pow(gp,3)+b*pow(gp,2)+c*gp+d
	return A

def fourth_poly(gp,a,b,c,d,e):
	A = a*pow(gp,4)+b*pow(gp,3)+c*pow(gp,2)+d*gp+e
	return A
...
### 	how to compute initial guesses?
popt_q,pcov_q = curve_fit(quadratic_poly, x1_I_vec,y_I_vec)#,p0=[4])
popt_c, pcov_c = curve_fit(cubic_poly, x1_I_vec,y_I_vec)#,p0=4)
popt_f, pcov_f = curve_fit(fourth_poly, x1_I_vec,y_I_vec)#,p0=4)

popt_e,pcov_e = curve_fit(exponential_poly, x1_I_vec,y_I_vec)

fit_q = quadratic_poly(x1_I_vec,*popt_q)
fit_c = cubic_poly(x1_I_vec, *popt_c)
fit_f = fourth_poly(x1_I_vec, *popt_f)

fit_e = exponential_poly(x1_I_vec,*popt_e) #need shift

plt.figure(3,figsize=(20,20))
plt.plot(x1_I_vec,y_I_vec,'.',markersize=mk10,label='raw data')
plt.plot(x1_I_vec,fit_q,'.',markersize=mk10,label='quadratic')
plt.plot(x1_I_vec,fit_c,'.',markersize=mk10,label='cubic')
plt.plot(x1_I_vec,fit_f,'.',markersize=mk10,label='4th')

plt.plot(x1_I_vec,fit_e,'.',markersize=mk10,label='exponential')
plt.title('curve fit')
plt.ylim(0,6)

popt_q_sorted, pcov_q_sorted = curve_fit(quadratic_poly, sorted_gp, sorted_BD)
curve_q = quadratic_poly(sorted_gp, *popt_q_sorted)

popt_c_sorted, pcov_c_sorted = curve_fit(cubic_poly, sorted_gp, sorted_BD)
curve_c = cubic_poly(sorted_gp,*popt_c_sorted)

popt_f_sorted, pcov_f_sorted = curve_fit(fourth_poly, sorted_gp, sorted_BD)
curve_f = fourth_poly(sorted_gp, *popt_f_sorted)

# plt.plot(sorted_gp, curve_BD,linewidth=4)
#r-squared:
r_q = r2_score(y_I_vec,fit_q)
r_c = r2_score(y_I_vec,fit_c)
r_f = r2_score(y_I_vec,fit_f)
r_e = r2_score(y_I_vec,fit_e)

#residuals
l = [fit_q,fit_c,fit_f,fit_e]
lr = [r_q,r_c,r_f,r_e]
li = ['fit_q','fit_c','fit_f','fit_e']
lc = [curve_q, curve_c, curve_f]
for i in range(0,len(l)-1):
	# plt.figure(figsize=(20,20))
	# plt.plot(x1_I_vec,y_I_vec,'.')
	# plt.plot(x1_I_vec,l[i],'.')
	plot_residuals(x1_I_vec,y_I_vec,l[i])
	plt.plot(sorted_gp,lc[i],linewidth=4)
	plt.title(li[i])
	plt.savefig(li[i])

...
def compute(list10):
	...
	# np.save(fileID+'_Itot_IpumpMax='+str(IpumpMax)+'_gp='+str(gp),I_mem)
		...
		## plot the data
			...
			f1 = 100
			##data label
			plt.text(-12, 35,r'I$_{pump}^{max}$='+str(IpumpMax)+' nA',fontsize=100,weight='bold')
			plt.text(-12,0,r'$\bar{g}_P$='+str(gp)+' nS',fontsize=100,weight='bold')
			plt.text(-10,-35,r'V',fontsize=f1,weight='bold')
			plt.text(-12,(0.08+referenceLine)*scale_factor1+shift1 + x1,r'I$_{pump}$',fontsize=f1,weight='bold')
			plt.text(-11,x2+13*scale_factor2+shift2,r'[Na]$_{in}$',fontsize=f1,weight='bold')
		...
		####################################### VOLTAGE TRACES
			...
			plt.plot([25,35],[-0.15*scale_factor3+shift3 +x3+15,-0.15*scale_factor3+shift3 +x3+15],linewidth=10,color='black')
			plt.text(27,-0.45*scale_factor3+shift3 +x3+10,'10 seconds')
			# labels for bars
			plt.text(-4.5,-30,'20 mV',fontsize=ft2,rotation=90)
			...
			plt.text(-4.5,10+x4-0.17*scale_factor4+shift4,'0.2 nA', fontsize=ft2,rotation=90)

			# labels for dashed lines
			plt.text(38,-50,'-50 mV', fontsize=ft2)
			plt.text(38,-4+referenceLine*scale_factor1+shift1 +x1,referenceStr, fontsize=ft2)
			plt.text(38,9.25*scale_factor2+shift2 +x2,'10 nM',fontsize=ft2)
			plt.text(38,-0.13*scale_factor4+shift4+x4,'-0.1 nA',fontsize=ft2)
	...
	# np.save(fileID+'_Itot_IpumpMax='+str(IpumpMax)+'_gp='+str(gp),I_mem)
		...
		####################################### VOLTAGE TRACES
			...
			plt.text(27,-0.45*scale_factor3+shift3 +x3-4,'10 seconds')
			# labels for bars
			plt.text(-4.5,-30,'20 mV',fontsize=ft2,rotation=90)
			plt.text(-4.5,(0.1+referenceLine)*scale_factor1+shift1 + x1,'0.1 nA', fontsize=ft2,rotation=90)
			plt.text(-4.5,13*scale_factor2+shift2 + x2,'5 nM', fontsize=ft2,rotation=90)

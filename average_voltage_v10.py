...
for i in range(0,len(experiment_list)):
	...
	if list10[-1] =='G':
		AV_gp = plt.figure(k,figsize=(25,25))
		AVgpax = plt.axes()
	if list10[-1] =='I':
		AV_pm = plt.figure(k,figsize=(25,25))
		AVpmax = plt.axes()			
...
for j in range(0,len(riff3)):
	...
	str1 = str(p1)
	plt.plot(0,0,'o',markersize=1e-3,color=color_riff3[j],label=str1+' nA')
plt.yticks(np.arange(-60,-10,20))
plt.xticks(np.arange(8,12,1.5))
plt.text(9,-65,r'$\bar{g}_P$='+str(ctr_prm[0])+' nS')
plt.legend(loc=2,bbox_to_anchor=(-0.2,1.15),markerscale=3e+4,ncol=3,fontsize=60)
...
plt.ylabel('V(mV)',weight='bold',fontsize=90)

plt.xlabel(r'$[Na^+]_{cyt}$ (mM)',weight='bold',fontsize=90)
plt.ylim(-70,-10)
plt.xlim(7.5,12)
AVgpax.set_frame_on(False)
plt.savefig(fileID+'_'+str(str_prm[0])+str(ctr_prm[0])+'V_Na_average'+'.png')
